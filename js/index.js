 ...
 *
 *             
 * Made By ProlightHub + on GitHub, this game is the original though I have had other accounts with it (Prolight, Nitrobyte)
...
var game = {
    gameState : "menu",
    sound : "off",
    debugMode : true, // Turning this on would be cheating!
    maxFps : 60,
    setFps : 60,
    fps : 60,
    fixedFps : true,
...
screenUtils.setBossBar = function(bossName, gameObjects)
{
...
var levels = {
    ...
    "Boss_Room" : {
        ...
        act : function()
        {
            screenUtils.setBossBar("redBeakerBoss", gameObjects);
        },
        plan : [
            "          ",
            "          ",
    ...
    "Ice_Boss_Room" : {
        ...
        act : function()
        {   
            screenUtils.setBossBar("iceBeakerBoss", gameObjects);
        },
        signs : {
            'a' : {
...
game.opts = function()
{
    ...
    if(mouseIsPressed && game.pressTimer <= 0)
    {
        game.opts.mousePressed();
        //mouseIsPressed = false;
        game.pressTimer = 10;
    }
    game.pressTimer--;
};
game.opts.mousePressed = function()
{
    ...
    switch(true)
    {
        case buttons.getButton("back").IsMouseInside() : 
              if(game.opts.optsLoadLevel)
              {
                  levelInfo.door = 'a';
                  game.opts.gameState = "play";
                  loader.startLevelLoading(levelInfo);
              }else{
                  game.opts.gameState = "menu";
                  game.opts.menu.setup();
              }
            break;
        ...
        case setButton.IsMouseInside() : 
              ...
              if(setButton.set)
              {
                  game.opts.debugMode = debugModeButton.is;
                  game.opts.optsLoadLevel = (levelInfo.level !== levelButton.level);
                  levelInfo.level = levelButton.level;
                  game.opts.fixedFps = fixedFpsButton.is;
              }else{
                  game.opts.debugMode = game.optsOriginal.debugMode;
                  game.opts.optsLoadLevel = game.optsOriginal.optsLoadLevel;
                  levelInfo.level = game.optsOriginal.level;
                  game.opts.fixedFps = game.optsOriginal.fixedFps;
              }
            break;
    }
};
...
draw = function() 
{
    ...
    if(game.fps !== this.lastGameFps)
    {
        frameRate(game.fps);
    }
    
    this.lastGameFps = game.fps;
